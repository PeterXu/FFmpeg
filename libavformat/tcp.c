...
static
int ijk_tcp_getaddrinfo_nonblock(URLContext *h, const char *hostname, const char *servname,
                                 ...
                                 const AVIOInterruptCB *int_cb, int one_by_one)
{
    ...
    while (1) {
        ...
        if (ret != 0 && ret != ETIMEDOUT) {
            av_log(h, AV_LOG_ERROR, "pthread_cond_timedwait failed: %d\n", ret);
            ret = AVERROR_EXIT;
            break;
        }
                                 ...
                                 const AVIOInterruptCB *int_cb, int one_by_one)
{
    ...
    if (!hostname || !hostname[0])
        return getaddrinfo(NULL, servname, hints, res);
    return getaddrinfo(hostname, servname, hints, res);
}
#endif /* HAVE_PTHREADS */
#endif /* CONFIG_IJK */
...
static int tcp_open(URLContext *h, const char *uri, int flags)
{
    ...
    if (s->dns_cache_timeout > 0) {
        if (s->dns_cache_clear) {
            av_log(h, AV_LOG_INFO, "will delete dns cache entry, uri = %s\n", uri);
            ...
            if (dns_entry && dns_entry->res && dns_entry->res->ai_family == AF_INET6 && !s->enable_ipv6) {
                ...
                remove_dns_cache_entry(uri);
                av_log(h, AV_LOG_INFO, "will delete dns cache entry because ipv6 fallback, uri = %s\n", uri);
                dns_entry = NULL;
            }
        }
    }
    ...
    dns_time = av_gettime();
    if (!dns_entry) {
        ret = ijk_tcp_getaddrinfo_nonblock(h, hostname, portstr, &hints, &ai, s->addrinfo_timeout, &h->interrupt_callback, s->addrinfo_one_by_one);
        if (ret) {
        ...
        av_log(h, AV_LOG_INFO, "hit dns cache uri = %s\n", uri);
 ...
 restart:
    ...
    if (cur_ai->ai_family == AF_INET6){
        ...
        av_log(h, AV_LOG_INFO, "cur ipv6 c_ipaddr = %s\n", c_ipaddr);
#endif
    }
#endif
#if CONFIG_IJK
    if (cur_ai->ai_family != AF_INET6 && cur_ai && cur_ai->ai_addr) {
        ipaddr = (struct sockaddr_in *)cur_ai->ai_addr;
        c_ipaddr = (char *)inet_ntop(AF_INET, &ipaddr->sin_addr, ipbuf, MAX_IP_LEN);
        av_log(h, AV_LOG_INFO, "cur ipv4 c_ipaddr = %s\n", c_ipaddr);
    }
    if (dns_entry) {
    ...
    if (s->listen == 2) {
        ...
        if (ret) {
            av_log(h, AV_LOG_WARNING, "terminated by application in AVAPP_CTRL_WILL_TCP_OPEN");
            goto fail1;
        }
        tcp_time = av_gettime();
        if ((ret = ff_listen_connect(fd, cur_ai->ai_addr, cur_ai->ai_addrlen,
            ...
            if (ret) {
                av_log(h, AV_LOG_WARNING, "terminated by application in AVAPP_CTRL_DID_TCP_OPEN");
                goto fail1;
            } else if (!dns_entry && !strstr(uri, control.ip) && s->dns_cache_timeout > 0) {
                add_dns_cache_entry(uri, cur_ai, s->dns_cache_timeout);
                av_log(h, AV_LOG_INFO, "add dns cache uri = %s, ip = %s port = %d\n", uri , control.ip, control.port);
            }
            av_log(h, AV_LOG_INFO, "tcp did open uri = %s, ip = %s port = %d\n", uri , control.ip, control.port);
        }
#else
...
#if CONFIG_IJK
    if (dns_entry) {
        av_log(h, AV_LOG_ERROR, "hit dns cache but connect fail uri = %s, ip = %s\n", uri , control.ip);
        release_dns_cache_reference(uri, &dns_entry);
        remove_dns_cache_entry(uri);
    } else {
        freeaddrinfo(ai);
    ...
    if (s->listen == 2) {
        ...
        if ((ret = ff_sendto(fd, http_request, strlen(http_request), FAST_OPEN_FLAG,
            ...
            if (ret) {
                ...
                add_dns_cache_entry(uri, cur_ai, s->dns_cache_timeout);
                av_log(h, AV_LOG_INFO, "add dns cache uri = %s, ip = %s\n", uri , control.ip);
            }
            av_log(h, AV_LOG_INFO, "tcp did open uri = %s, ip = %s\n", uri , control.ip);
        }
    }
...
static int tcp_write(URLContext *h, const uint8_t *buf, int size)
{
    ...
    if (s->fastopen && !s->tcp_connected && av_stristart(buf, "GET", NULL)) {
        ...
        if (!ret) {
            ...
            av_log(h, AV_LOG_WARNING, "tcp_fast_open is error ret = %d\n", ret);
            return ret;
        }
    }
#endif
